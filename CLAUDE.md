...
- `index.html` — Home: Hero (Symbiotic relief) → `#works` "Portfolio" section with three aligned preview cards (first three series, object photographs, square crop via `aspect-ratio`) → About → Footer (Connect column holds Email / Instagram; there is no Contact section)
...
- Footer "Elsewhere" links: Keli Jewellery (Shopify store, described as "Online shop") → https://kelijewellery.com/, Orris → https://orris.kelijewellery.com/ (both pages).
